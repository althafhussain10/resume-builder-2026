...
export const ModernTemplate = ({ data }: Props) => (
  <div className="flex text-[11px]" style={{ fontFamily: "Calibri, sans-serif" }}>
    ...
    <div className="w-2/3 p-6">
      ...
      <div><h2 className="text-xs font-bold uppercase text-blue-600 mb-2">Education</h2>{data.education.map(e => (<div key={e.id} className="mb-2"><p className="font-bold">{e.institution}</p><p className="text-gray-600">{e.degree} in {e.field} | {e.startDate} - {e.endDate}</p></div>))}</div>
    </div>
  </div>
);
